...
const CategoryOverviewCard: React.FC<CategoryOverviewCardProps> = ({
  ...
  return (
    <Card className="glassmorphic-card text-foreground animate-in fade-in slide-in-from-bottom-2 duration-300"> {/* Applied consistent card style */}
      <motion.div
      ...
      >
        <CardHeader className="p-4 sm:p-5 lg:p-6 pb-2"> {/* Applied consistent padding */}
          <CardTitle className="text-base sm:text-lg font-semibold tracking-tight flex items-center"> {/* Applied consistent typography */}
            <PiggyBank className="w-5 h-5 mr-2 text-primary flex-shrink-0" /> <span className="min-w-0 truncate">{t("dashboard.budgetCategories")}</span>
          </CardTitle>
          <Link to="/budget-app?view=budget" className="text-xs sm:text-sm text-primary dark:text-primary hover:text-primary/90 dark:hover:bg-primary/90 font-medium flex items-center flex-shrink-0"> {/* Applied consistent typography */}
            {t("common.viewAll")}
        ...
        <CardContent className="p-4 sm:p-5 lg:p-6 pt-0"> {/* Applied consistent padding */}
          <div className="flex flex-col sm:flex-row items-center justify-between mb-6">
            <div className="w-full sm:w-1/2 h-[150px] flex items-center justify-center mb-4 sm:mb-0 relative">
              {totalBudgetedMonthly > 0 ? (
                ...
                <p className="text-sm sm:text-base text-center text-muted-foreground break-words text-balance">{t("dashboard.noBudgetYet")}</p> // Applied consistent typography and text wrapping
              )}
            </div>
          ...
          <div className="mt-4 border-t border-border/10 pt-4 divide-y divide-border/10"> {/* Applied consistent divider */}
            <h3 className="text-base sm:text-lg font-semibold tracking-tight mb-3">{t("dashboard.yourCategories")}</h3> {/* Applied consistent typography */}
            <div className="space-y-3">
              {categories.length > 0 ? (
                categories.map((cat) => {
                  ...
                  return (
                    <div key={cat.id} className="flex flex-col space-y-1 py-2"> {/* Added py-2 for consistent spacing */}
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2 min-w-0"> {/* Added min-w-0 */}
                          <span className="text-lg flex-shrink-0">{cat.emoji}</span>
                          <span className="font-medium text-foreground text-sm sm:text-base truncate">{cat.name}</span> {/* Applied consistent typography and truncate */}
                        </div>
                        <div className="text-sm sm:text-base text-right flex items-center space-x-1 font-mono flex-shrink-0"> {/* Applied consistent typography */}
                ...
                <p className="text-sm sm:text-base text-muted-foreground text-center break-words text-balance">{t("dashboard.noCategoriesDefined")}</p> // Applied consistent typography and text wrapping
              )}
            </div>
          </div>
        </CardContent>
